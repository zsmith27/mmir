...
* 0 = The medians of both the refrence and degraded conditions overlap the interquartile range of one another.
...
Discrimination Efficiency (DE) is a measure of the percentage of degraded samples correctly identified below the reference distributions 25th percentile for metrics that decrease with degredation or the percentage of degraded samples correctly identified above the reference distributions 75th percentile for metrics that increase with disturbance.

Balanced Discrimination Efficiency (BDE) attempts to update the DE method by throwing more computation power at the problem. BDE tests multiple thresholds for seperating the reference and degraded distributions in an attempt to find the best seperation point.

Use the `sensitivity()` function to calculate three types of metric senstivity measures: 1) Barbour et al. (1996) method, 2) Descrimination Efficiency (DE), and 3) Balanced Descrimination Efficiency (BDE). 
```{r}
