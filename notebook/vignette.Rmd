...
rich.df <- onondaga %>% 
  group_by(unique_id) %>% 
  dplyr::do(
    rich_family = taxa_rich(long.df = ., low.taxa.col = family),
    rich_genus = taxa_rich(long.df = ., low.taxa.col = genus)
  )
...
subrich.df <- onondaga %>% 
  group_by(unique_id) %>% 
  dplyr::do(
    rich_ephemeroptera_fam = taxa_rich(long.df = ., 
                                       low.taxa.col = order,
                                       high.taxa.col = family,
                                       taxon = "ephemeroptera"),
    rich_ephemeroptera_gen = taxa_rich(long.df = .,
                                       low.taxa.col = order,
                                       high.taxa.col = genus,
                                       taxon = "ephemeroptera"),
    rich_ept_gen = taxa_rich(long.df = .,
                             low.taxa.col = order,
                             high.taxa.col = genus,
...
pctrich.df <- onondaga %>% 
  group_by(unique_id) %>% 
  dplyr::do(
    rich_ephemeroptera_fam = taxa_pct_rich(long.df = ., 
                                       low.taxa.col = order,
                                       high.taxa.col = family,
                                       taxon = "ephemeroptera"),
    rich_ephemeroptera_gen = taxa_pct_rich(long.df = .,
                                       low.taxa.col = order,
                                       high.taxa.col = genus,
                                       taxon = "ephemeroptera"),
    rich_ept_gen = taxa_pct_rich(long.df = .,
                             low.taxa.col = order,
                             high.taxa.col = genus,
                             taxon = c("ephemeroptera",
                                       "plecoptera",
...
DT::datatable(pctrich.df, options = list(scrollX = TRUE))  
```
...
div.df <- onondaga %>% 
  ...
  dplyr::do(
    shannon_genus = taxa_div(long.df = .,
                             count.col = reporting_value,
                             high.taxa.col = genus,
                             job = "shannon",
                             base.log = 2),
    simpson_genus = taxa_div(long.df = ., 
                             count.col = reporting_value,
                             high.taxa.col = genus,
                             job = "simpson"),
    margalef_genus = taxa_div(long.df = .,
                              count.col = reporting_value,
                              high.taxa.col = genus,
                              job = "margalef"),
    menhinick_genus = taxa_div(long.df = .,
                               count.col = reporting_value,
                               high.taxa.col = genus,
                               job = "menhinick"),
    pielou_genus = taxa_div(long.df = .,
                            count.col = reporting_value,
                            high.taxa.col = genus,
                            job = "pielou")
  )
