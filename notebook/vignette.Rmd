...
```{r}
rich.df <- onondaga %>% 
  dplyr::group_nest(unique_id) %>% 
  dplyr::mutate(
    rich_family = taxa_rich(long.df = ., 
                            unique.id.col = unique_id,
                            low.taxa.col = order, 
                            high.taxa.col = family,
                            taxon = NULL),
    rich_genus = taxa_rich(long.df = .,
                           unique.id.col = unique_id,
                           low.taxa.col = order,
                           high.taxa.col = genus,
                           taxon = NULL)
...
pct.df <- metrics.key %>% 
  dplyr::mutate(
    pct_ephemeroptera = taxa_pct(long.df = onondaga,
                                  unique.id.col = unique_id,
                                  count.col = reporting_value,
                                  taxon.col = order,
                                  taxon = "ephemeroptera"),
    pct_ept = taxa_pct(onondaga,
                        unique.id.col = unique_id,
                        count.col = reporting_value,
                        taxon.col = order,
                        taxon = c("ephemeroptera", "plecoptera", "trichoptera"))
...
abund.df <- metrics.key %>% 
  dplyr::mutate(
    abund_ephemeroptera = taxa_abund(onondaga,
                                  unique.id.col = unique_id,
                                  count.col = reporting_value,
                                  taxon.col = order,
                                  taxon = "ephemeroptera"),
    abund_ept = taxa_abund(onondaga,
                        unique.id.col = unique_id,
                        count.col = reporting_value,
                        taxon.col = order,
                        taxon = c("ephemeroptera", "plecoptera", "trichoptera"))
